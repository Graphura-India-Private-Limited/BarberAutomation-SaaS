    ...
    fetch(`${API}/payment?${params}`, {
      ...
      .then(next => {
        if (active) {
          ...
          else throw new Error(next.message || "Failed to load payments");
        }
      })
    ...
    <div className="max-w-5xl mx-auto text-left font-sans animate-fade-in">
      ...
      <div className="flex justify-end items-center gap-6 mb-4 font-sans">
        <div className="flex items-center gap-1.5 bg-white px-3 py-1.5 rounded-xl border border-stone-200/60 shadow-sm">
          <ClockIcon />
          <span className="text-[10px] font-extrabold uppercase tracking-widest text-[#C5A059]">System Clock: <span className="text-stone-800 font-mono tracking-normal">{time} IST</span></span>
        </div>
      </div>

      {/* Context Header Title Card Block */}
      <div className="relative rounded-3xl p-6 md:p-8 mb-8 overflow-hidden card bg-white text-left">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <button 
            onClick={() => setPage(1)} 
            className="inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3.5 text-xs font-extrabold tracking-wider uppercase text-white shadow-md active:scale-[0.98] transition-all duration-200 self-start md:self-center cursor-pointer hover:opacity-90 font-sans border-none"
      ...
      <section className="mb-6 grid gap-4 p-5 card md:grid-cols-5 bg-white shadow-sm text-left">
        ...
        <label className="text-[11px] font-extrabold uppercase tracking-widest text-[#C5A059] font-sans">
          ...
          <div className="mt-2 flex items-center gap-2 rounded-xl border border-stone-200 bg-stone-50/50 px-3 py-2.5 hover:bg-white focus-within:border-[#C5A059] focus-within:bg-white transition-all">
            ...
            <input type="date" value={filters.date} onChange={e => updateFilter("date", e.target.value)} className="w-full bg-transparent text-xs outline-none text-stone-800 font-bold font-sans border-none" />
          </div>
        </label>
        ...
        <FilterSelect
          ...
          options={[{ id: "", name: "All Barbers" }, ...barbersList.map(b => ({ id: b._id, name: b.name }))]}
          objectOptions
        />
        ...
        <label className="text-[11px] font-extrabold uppercase tracking-widest text-[#C5A059] font-sans">
          ...
          <div className="mt-2 flex items-center gap-2 rounded-xl border border-stone-200 bg-stone-50/50 px-3 py-2.5 hover:bg-white focus-within:border-[#C5A059] focus-within:bg-white transition-all">
            ...
            <input value={filters.q} onChange={e => updateFilter("q", e.target.value)} placeholder="Order or payment ID" className="w-full bg-transparent text-xs outline-none text-stone-800 font-bold placeholder-stone-400 font-sans border-none" />
          </div>
        </label>
      </section>
      ...
      <section className="card overflow-hidden bg-white shadow-sm text-left font-sans">
        <div className="hidden grid-cols-[1.4fr_1fr_1fr_1fr_1fr_0.8fr] border-b border-stone-100 bg-stone-50/50 px-6 py-4 text-[10px] font-extrabold uppercase tracking-widest text-[#C5A059] md:grid">
  ...
  return (
    <div className="grid gap-3 p-6 md:grid-cols-[1.4fr_1fr_1fr_1fr_1fr_0.8fr] md:items-center hover:bg-stone-50/30 transition-colors group text-left font-sans">
      <div>
        ...
        <p className="mt-1 text-[10px] text-stone-400 font-medium font-sans">{fmtDate(payment.created_at)}</p>
      </div>
      <p className="text-sm font-bold text-stone-900 tracking-tight">{payment.customer_id?.name || "Customer Walk-in"}</p>
      <p className="text-xs font-bold text-stone-505 tracking-wide">{payment.barber_id?.name || "Unassigned"}</p>
      <div>
      ...
      <div className="flex justify-start gap-2 md:justify-end border-t md:border-0 pt-3 md:pt-0 border-stone-100 font-sans">
        ...
        {payment.status === "FAILED" && <button onClick={onRetry} className="rounded-xl hover:opacity-95 px-4 py-2 text-xs font-extrabold tracking-wider uppercase text-white shadow-md transition-all cursor-pointer border-none" style={{ background: CHARCOAL }}>Retry</button>}
      </div>
    </div>
  );
}
...
function Detail({ label, value, highlight = false }) {
  return (
    <div className="bg-[#FAF6F0]/60 rounded-xl p-4 border border-[#EADBCE]/60 transition-all text-left">
      <p className="text-[10px] font-extrabold uppercase tracking-widest text-[#C5A059] font-sans">{label}</p>
