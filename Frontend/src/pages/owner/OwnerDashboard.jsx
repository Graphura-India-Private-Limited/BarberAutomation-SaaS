    ...
    <div className="max-w-5xl mx-auto font-sans">
      ...
      <header className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 border-b pb-6 border-stone-200 text-left">
        ...
        <div className="flex items-center gap-3 font-sans">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-5 py-3.5 rounded-xl font-extrabold text-xs tracking-wider text-white uppercase shadow-sm transition-all active:scale-95 cursor-pointer font-sans border-none outline-none"
      ...
      {!editing && (
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 text-left">
          <div onClick={() => navigate("/owner/revenue")} className="card p-5 flex items-center gap-4 bg-white shadow-2xs cursor-pointer">
            ...
            <div>
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-[#C5A059] mb-0.5 font-sans">Today's Revenue</p>
              <h3 className="text-xl font-black text-stone-900 font-serif">₹{salon?.basic_pricing ? salon.basic_pricing * 14 : "8,450"}</h3>
            </div>
          </div>
          <div onClick={() => navigate("/owner/queue")} className="card p-5 flex items-center gap-4 bg-white shadow-2xs cursor-pointer">
            ...
            <div>
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-[#C5A059] mb-0.5 font-sans">Live Queue</p>
          ...
          <div onClick={() => navigate("/owner/barbers")} className="card p-5 flex items-center gap-4 bg-white shadow-2xs cursor-pointer">
            <div className="w-11 h-11 rounded-xl bg-emerald-50 border border-emerald-100 flex items-center justify-center shrink-0">
              <Activity className="text-emerald-600 w-5 h-5" />
            </div>
            <div>
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-[#C5A059] mb-0.5 font-sans">Active Staff</p>
          ...
          <div onClick={() => navigate("/owner/queue")} className="card p-5 flex items-center gap-4 bg-white shadow-2xs cursor-pointer">
            ...
            <div>
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-[#C5A059] mb-0.5 font-sans">Avg Wait Time</p>
        ...
        {editing ? (
          ...
          <>
              ...
              <div className="card p-6">
                ...
                <div className="grid grid-cols-3 gap-2.5">
                  {(form.images || []).map((url, index) => (
                    <img key={index} src={url} alt={`Preview ${index + 1}`} className="aspect-square rounded-xl object-cover border border-stone-100 w-full" />
                  ))}
                  {(!form.images || form.images.length === 0) && (
                    <div className="col-span-3 py-8 rounded-xl border border-dashed border-stone-200 text-center text-[10px] font-extrabold uppercase tracking-widest text-[#C5A059] font-sans">
            ...
            <section className="space-y-6 lg:col-span-7">
              ...
              <div className="card p-6 flex flex-col justify-between bg-white" style={{ minHeight: "260px" }}>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-[10px] font-extrabold uppercase tracking-widest text-amber-800 bg-amber-50 px-2.5 py-1 rounded-md border border-amber-200/50 font-sans">This Week ₹52,300</span>
                </div>

                <div className="flex justify-between items-end h-32 pt-4 px-2 font-sans">
                  {[
                    ...
                    <div key={i} className="flex flex-col items-center justify-end h-full flex-1 group gap-2">
                      ...
                      <div className="w-6 sm:w-8 rounded-t-md transition-all duration-300" style={{ height: item.h, backgroundColor: item.active ? GOLD : "#FAF1E6" }} />
                      <span className="text-[10px] font-extrabold uppercase tracking-widest text-stone-400 font-sans mt-1">{item.day}</span>
                    </div>
                  ))}
                </div>
              </div>
...
function Info({ label, value }) {
  return (
    <div className="border-b last:border-0 pb-2.5 last:pb-0 text-left border-stone-50">
      <label className="text-[10px] font-extrabold uppercase tracking-widest text-[#C5A059] block font-sans">{label}</label>
...
function ProfileEditor({ form, setField, addImages, tagLocation, saveProfile, busy, canResubmit }) {
  ...
  return (
    <div className="card p-6 shadow-md bg-white text-left font-sans">
      ...
      <div className="mt-5 flex flex-wrap gap-3 pt-4 border-t border-stone-50 font-sans">
        <button type="button" onClick={tagLocation} className="rounded-xl border border-[#C5A059] text-[#C5A059] px-5 py-3 text-xs font-extrabold uppercase tracking-wider flex items-center gap-1.5 cursor-pointer font-sans bg-transparent"><MapPin size={14} /> Tag GPS Geolocation</button>
        <button type="button" onClick={() => saveProfile(false)} disabled={busy} className="rounded-xl bg-stone-900 text-white px-6 py-3 text-xs font-extrabold uppercase tracking-wider hover:bg-stone-800 disabled:opacity-40 shadow-sm cursor-pointer font-sans border-none">Save Staged Changes</button>
        {canResubmit && <button type="button" onClick={() => saveProfile(true)} disabled={busy} className="rounded-xl px-6 py-3 text-xs font-extrabold uppercase tracking-wider text-white shadow-sm cursor-pointer font-sans border-none" style={{ background: GOLD }}>Resubmit Request</button>}
      </div>
    </div>
  );
}