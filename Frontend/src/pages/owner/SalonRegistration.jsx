...
import React, { useState } from "react";
import { useNavigate } from "react-router-dom";
import { ArrowLeft, MapPin, Upload, AlertTriangle, ShieldCheck, ChevronDown, X, Eye } from "lucide-react";
import shopImage from "../../assets/shop.jpg";
...
// Allowed document upload types: JPG, PNG, PDF only
const ALLOWED_DOC_MIME_TYPES = ["image/jpeg", "image/png", "application/pdf"];
const ALLOWED_DOC_EXTENSIONS = [".jpg", ".jpeg", ".png", ".pdf"];
const DOC_ACCEPT_ATTR = "image/jpeg,image/png,application/pdf,.jpg,.jpeg,.png,.pdf";
const FILE_TYPE_ERROR_MESSAGE = "Only JPG, PNG, and PDF files are allowed.";
const MAX_DOC_SIZE_BYTES = 5 * 1024 * 1024; // 5MB
const FILE_TOO_LARGE_MESSAGE = "File is too large. Maximum size is 5MB.";

// NOTE: Client-side checks below are a UX convenience only, NOT a security
// boundary. They can be bypassed by anyone calling the API directly, so the
// backend MUST independently validate file type (via magic-byte/content
// sniffing, not just extension or the browser-reported MIME type) and size
// before persisting or serving any uploaded document.
const isAllowedDocFile = (file) => {
  if (!file) return false;
  const nameLower = file.name ? file.name.toLowerCase() : "";
  const hasAllowedExtension = ALLOWED_DOC_EXTENSIONS.some(ext => nameLower.endsWith(ext));
  const hasAllowedMime = ALLOWED_DOC_MIME_TYPES.includes(file.type);
  // Require BOTH a matching extension AND a matching MIME type, instead of
  // either one alone, to make simple rename-based spoofing harder.
...
const emptyForm = {
  ...
  gst_certificate: "",
  aadhaar_card: "",
  shop_establishment_certificate_name: "",
  trade_license_name: "",
...
export default function SalonRegistration() {
  ...
  const handleImages = async (event) => {
    ...
    // Restrict gallery uploads to JPG/PNG as well for consistency with document uploads.
    const validFiles = files.filter(file => {
      const nameLower = file.name ? file.name.toLowerCase() : "";
      const hasAllowedExtension = [".jpg", ".jpeg", ".png"].some(ext => nameLower.endsWith(ext));
      const hasAllowedMime = ["image/jpeg", "image/png"].includes(file.type);
      return hasAllowedMime && hasAllowedExtension;
    });
  ...
  // Opens a previously-uploaded document (stored as a base64 data URL) in a
  // new browser tab so the user can confirm what they attached.
  const previewDocument = (dataUrl) => {
    if (!dataUrl) return;
    const win = window.open();
    if (win) {
      win.document.write(
        `<iframe src="${dataUrl}" style="border:0;width:100%;height:100%;" title="Document preview"></iframe>`
      );
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const successMessage = "Your salon profile has been submitted successfully. Our team will review your application and contact you within 3–4 business days via email or phone.";
      setMessage(successMessage);
      setSubmitted(true);
      // Give the user a moment to actually read the confirmation message
  ...
  if (submitted) {
    return (
      <div
        className="relative min-h-screen bg-cover bg-center bg-fixed p-4 sm:p-6 font-sans text-stone-800 flex flex-col items-center justify-center"
        style={{ backgroundImage: `url(${shopImage})` }}
      >
        <style>{`
          @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&family=Playfair+Display:ital,wght@0,400..900;1,400..900&display=swap');
        ...
        `}</style>
        <div className="absolute inset-0 bg-[#FAF6F0]/85 backdrop-blur-xs" />
        <div className="relative z-10 w-full max-w-xl card-premium p-8 sm:p-12 text-center space-y-6">
          ...
          <p className="text-sm sm:text-base leading-relaxed text-stone-600 font-sans">
            {message}
          </p>
          <p className="text-xs text-stone-400 font-sans">
            Redirecting you to the Owner Login page...
          </p>
          <button
            type="button"
            onClick={() => navigate("/owner/login")}
            className="w-full h-12 rounded-2xl flex items-center justify-center text-xs sm:text-sm font-extrabold uppercase tracking-wider text-white shadow-md transition-all duration-200 hover:opacity-95 cursor-pointer font-sans"
            style={{ backgroundColor: CHARCOAL }}
          >
            Go to Owner Login
          </button>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <div className="md:col-span-2 space-y-4">
              <div className="border-b border-[#EADBCE] pb-2">
                ...
                <p className="text-stone-500 text-xs mt-1 font-sans">
                  Please upload valid copies of the requested documents. Only JPG, PNG, and PDF formats are accepted (max 5MB each). Click an uploaded file to preview it.
                </p>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Shop & Establishment Certificate *">
                  <DocUploadControl
                    inputId="shop-establishment-upload"
                    dataUrl={form.shop_establishment_certificate}
                    fileName={form.shop_establishment_certificate_name}
                    placeholder="Upload Certificate..."
                    onChoose={() => triggerFileInput("shop-establishment-upload")}
                    onFileSelected={e => handleDocumentUpload(e, "shop_establishment_certificate")}
                    onPreview={() => previewDocument(form.shop_establishment_certificate)}
                    onClear={() => clearDocument("shop_establishment_certificate")}
                  />
                </Field>
                ...
                <Field label="Trade License *">
                  <DocUploadControl
                    inputId="trade-license-upload"
                    dataUrl={form.trade_license}
                    fileName={form.trade_license_name}
                    placeholder="Upload License..."
                    onChoose={() => triggerFileInput("trade-license-upload")}
                    onFileSelected={e => handleDocumentUpload(e, "trade_license")}
                    onPreview={() => previewDocument(form.trade_license)}
                    onClear={() => clearDocument("trade_license")}
                  />
                </Field>
                ...
                <Field label="GST Certificate *">
                  <DocUploadControl
                    inputId="gst-certificate-upload"
                    dataUrl={form.gst_certificate}
                    fileName={form.gst_certificate_name}
                    placeholder="Upload GST Certificate..."
                    onChoose={() => triggerFileInput("gst-certificate-upload")}
                    onFileSelected={e => handleDocumentUpload(e, "gst_certificate")}
                    onPreview={() => previewDocument(form.gst_certificate)}
                    onClear={() => clearDocument("gst_certificate")}
                  />
                </Field>
                ...
                <Field label="Owner's Aadhaar Card *">
                  <DocUploadControl
                    inputId="aadhaar-card-upload"
                    dataUrl={form.aadhaar_card}
                    fileName={form.aadhaar_card_name}
                    placeholder="Upload Aadhaar Card..."
                    onChoose={() => triggerFileInput("aadhaar-card-upload")}
                    onFileSelected={e => handleDocumentUpload(e, "aadhaar_card")}
                    onPreview={() => previewDocument(form.aadhaar_card)}
                    onClear={() => clearDocument("aadhaar_card")}
                  />
                </Field>
              </div>
            </div>
            ...
            <Field label="Shop Gallery Images (Max 5)">
              <div className="relative w-full h-[50px]">
                <input type="file" id="gallery-uploads" accept="image/jpeg,image/png,.jpg,.jpeg,.png" multiple onChange={handleImages} className="hidden" />
                <button
                  type="button"
                  onClick={() => triggerFileInput("gallery-uploads")}
                  className="w-full h-full bg-white border border-[#EADBCE] rounded-2xl px-4 flex items-center justify-between text-stone-400 text-sm font-medium font-sans shadow-3xs hover:border-[#C5A059] transition-colors cursor-pointer"
              ...
              {form.images.map((image, index) => (
                <div key={index} className="relative group aspect-square">
                  <img 
                    src={image} 
                    alt={`Salon upload ${index + 1}`} 
                    className="w-full h-full rounded-2xl border border-[#EADBCE] object-cover shadow-3xs transition-all duration-200 group-hover:brightness-95 animate-in fade-in zoom-in-95 cursor-pointer" 
                    onClick={() => previewDocument(image)}
                  />
                  <button
                    type="button"
                    onClick={() => {
...
// Reusable upload control for a single verification document: lets the user
// choose a file, shows the picked file name, and lets them click to preview
// or clear what they uploaded.
function DocUploadControl({ inputId, dataUrl, fileName, placeholder, onChoose, onFileSelected, onPreview, onClear }) {
  return (
    <div className="relative w-full">
      <input
        type="file"
        ...
        className="hidden"
      />
      <div className="w-full h-[50px] bg-white border border-[#EADBCE] rounded-2xl pl-4 pr-2 flex items-center justify-between text-stone-400 text-sm font-medium font-sans shadow-3xs hover:border-[#C5A059] transition-colors">
        {dataUrl ? (
          <button
            type="button"
            onClick={onPreview}
          ...
          >
            <Eye size={14} color={GOLD} className="shrink-0" />
            <span className="text-stone-600 text-xs font-normal truncate">
          ...
          <button
            type="button"
            onClick={onChoose}
            className="flex-grow text-left text-stone-500 text-xs font-normal truncate cursor-pointer"
          >
            {placeholder}
          </button>
        )}
        ...
        <div className="flex items-center gap-1 shrink-0 pl-2">
          {dataUrl ? (
            <button
              ...
              className="text-stone-400 hover:text-red-500 transition-colors p-1.5 cursor-pointer"
            >
              <X size={14} />
            </button>
          ) : (
