...
export default function Wrapper() {
  ...
  const handleResetFlow = () => {
    ...
    setBookingData({ service: 'Haircut & Styling', price: 500, barber: 'Rahul', date: null, time: null });
    setCurrentStep(1);
    const token = localStorage.getItem("token");
  ...
  const handleHeaderBackAction = () => {
    const token = localStorage.getItem("token");
    if (currentStep === 3) navigate(token ? "/dashboard" : "/");
    else if (currentStep === 4) setCurrentStep(3);
    else if (currentStep === 5) setCurrentStep(4);
    else if (currentStep === 6) handleResetFlow();
  };
    ...
    <>
      ...
      <div className="bg-[#FAF6F0] min-h-screen font-sans text-[#3E362E] selection:bg-[#C5A059] selection:text-white relative overflow-hidden flex flex-col justify-between">
        ...
        <div className="w-full flex-grow pt-24">
          {currentStep >= 3 && (
            <div className="w-full max-w-7xl mx-auto px-6 pt-4 relative z-50 flex justify-start">
              ...
              >
                ...
                <span>{currentStep === 3 ? "Exit Booking" : currentStep === 6 ? "Home" : "Back"}</span>
              </button>
            </div>
          )}
          ...
          <div className="max-w-2xl mx-auto px-4 pb-20 sm:pb-24 relative z-30 w-full">
            <div className="bg-white/90 backdrop-blur-md rounded-[32px] p-6 sm:p-10 border border-[#EADDCA] shadow-[0_10px_30px_rgba(0,0,0,0.01)] transition-all duration-500">
              ...
              {currentStep === 5 && <CheckoutPage bookingData={bookingData} onBack={() => setCurrentStep(4)} onComplete={handleCheckoutComplete} />}
              {currentStep === 6 && (
                <ConfirmationPage
