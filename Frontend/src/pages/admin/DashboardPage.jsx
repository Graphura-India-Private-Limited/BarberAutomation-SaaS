...
export function DashboardPage({ tickets, onSelectTicket }) {
  ...
  const fetchSubscribers = async () => {
    ...
    try {
      ...
      else setSubError(data.message || "Failed to load subscribers.");
    } catch {
  ...
  const handleDeleteSubscriber = async (id) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) setSubscribers(prev => prev.filter(s => s._id !== id));
  ...
  return (
    <div className="p-6 space-y-8 text-stone-800 animate-in fade-in duration-300">
      ...
      {/* ── CRITICAL ALERT ── */}
      {stats.critical > 0 && (
        <div className="bg-rose-50/60 border border-rose-200 rounded-2xl px-5 py-4 flex items-start gap-3.5">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-5 border-b border-stone-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              {!subLoading && (
                <span className="ml-2 px-2.5 py-0.5 rounded-full text-[11px] font-black"
          ...
          <div className="px-6 py-4">
            {subLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <button onClick={fetchSubscribers} className="mt-3 text-[11px] font-bold underline underline-offset-2"
                  style={{ color: GOLD }}>
                  Try again
                </button>
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-[11px] text-stone-400 mt-1">
                  {subSearch ? "Try a different search term." : "Emails submitted via the footer will appear here."}
                </p>
              </div>
            ) : (
  ...
  return (
    <div className={`card p-5 flex items-center gap-4 shadow-2xs ${bg} ${border}`}>
      ...
      <div className="flex-1 min-w-0 text-left">
        ...
        <h3 className={`text-2xl font-black font-serif leading-none tracking-tight ${isCritical ? "text-rose-700" : "text-stone-900"}`}>
          {value}
        </h3>
