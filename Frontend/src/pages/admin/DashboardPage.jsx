...
/* ── ✅ FIXED: ISOLATED STRUCTURAL METRIC COMPONENT CELL ── */
  ...
  return (
    <div className={`card p-5 flex items-center gap-4 shadow-2xs ${bg || ''} ${border || ''}`}>
      ...
      <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${
        ...
        {/* ✅ FIXED Style conditions & stripped out ghost whitespace tokens */}
        {Icon && (
          <Icon 
            size={18} 
            strokeWidth={2.5} 
            className={isCritical ? 'text-rose-600' : 'text-stone-500'} 
            style={{ color: isCritical ? '#E11D48' : GOLD }} 
          />
        )}
      </div>
      ...
      <div className="flex-1 min-w-0 text-left">
        ...
        {/* ✅ FIXED: Backticks added cleanly for template string evaluation */}
        <h3 className={`text-2xl font-black font-serif leading-none tracking-tight ${
          isCritical ? 'text-rose-700' : 'text-stone-900'
        }`}>
          {value}
        </h3>
        
        {sub && (
          <p className="text-[9px] font-bold text-stone-400 mt-1 truncate font-sans uppercase tracking-wide">
            {sub}
          </p>
        )}
      </div>
