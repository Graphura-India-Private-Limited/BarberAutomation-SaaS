...
import { User, ShieldCheck, Briefcase, Phone, Award, Save, X, Star } from "lucide-react";
  ...
  return (
    <div className="w-full text-stone-800 font-sans antialiased flex flex-col">
      ...
      <main className="max-w-5xl mx-auto w-full px-6 py-10 flex-1 text-left">
        ...
        {/* Tab Navigation */}
        <div className="flex gap-2 border-b border-stone-200 mb-8 pb-px">
          <button
            type="button"
            onClick={() => setActiveTab("profile")}
            className={`py-3 px-6 text-xs font-black uppercase tracking-widest border-b-2 transition-all duration-200 flex items-center gap-2 cursor-pointer outline-none ${
              activeTab === "profile"
                ? "border-[#C5A059] text-[#3E362E] bg-stone-100/50 rounded-t-xl font-bold"
                : "border-transparent text-stone-400 hover:text-stone-600 hover:bg-stone-50/50 rounded-t-xl"
            }`}
          >
            <User size={14} className={activeTab === "profile" ? "text-[#C5A059]" : ""} />
            Profile Details
          </button>
          <button
            type="button"
          ...
          >
            <Star size={14} className={activeTab === "reviews" ? "text-[#C5A059]" : ""} />
            Client Reviews
          </button>
        </div>
